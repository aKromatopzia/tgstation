...
/obj/item/organ/heart/gland/heal/proc/replace_chest(obj/item/bodypart/chest/chest)
	if(!IS_ORGANIC_LIMB(chest))
		...
		for(var/i in 1 to 3)
			var/obj/effect/decal/cleanable/vital/robotic/debris/debris = new(get_turf(owner))
			debris.streak(dirs)
	else
		...
		var/list/dirs = GLOB.alldirs.Copy()
		for(var/i in 1 to 3)
			var/obj/effect/decal/cleanable/vital/organic/gibs/gibs = new(get_turf(owner))
			gibs.streak(dirs)
